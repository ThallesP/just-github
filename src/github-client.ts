...
export class GitHubClient {
  ...
  async fetchContents(path: string): Promise<GitHubContentResponse | GitHubContentResponse[]> {
    ...
    const url = `${this.baseUrl}/repos/${this.owner}/${this.repo}/contents/${normalized}?ref=${this.ref}`;
  ...
  async fetchTree(): Promise<GitHubTreeResponse> {
    ...
    const refData = await this.request<{ object: { sha: string; type: string } }>(
      `${this.baseUrl}/repos/${this.owner}/${this.repo}/git/ref/heads/${this.ref}`,
      this.ref,
    ).catch(async () => {
      // Try as a tag
      return this.request<{ object: { sha: string; type: string } }>(
        `${this.baseUrl}/repos/${this.owner}/${this.repo}/git/ref/tags/${this.ref}`,
        this.ref,
      );
    }).catch(async () => {
      // Try as a direct commit SHA — get the commit directly
      const commit = await this.request<{ tree: { sha: string } }>(
        `${this.baseUrl}/repos/${this.owner}/${this.repo}/git/commits/${this.ref}`,
        this.ref,
      );
    ...
    if ("_commit" in refData) {
      treeSha = (refData as any)._commit.tree.sha;
